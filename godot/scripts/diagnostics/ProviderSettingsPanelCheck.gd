...
func _run() -> void:
	...
	var advanced: Object = panel.get("_advanced")
	_expect(advanced != null, "高级/生活分节控制器不存在")
	var knowledge: Object = panel.get("_knowledge")
	_expect(knowledge != null, "知识库分节控制器不存在")
	var provider: Object = panel.get("_provider")
	_expect(provider != null, "AI 服务分节控制器不存在")
	...
	_expect(provider.get("_provider_api_key_input") == null, "体验页仍堆叠显示 AI Provider")
	_expect(advanced.get("_diagnostic_bundle_button") == null, "体验页仍堆叠显示开发者诊断入口")

	panel.call("_switch_settings_category", "advanced")
	_expect(str(panel.get("_settings_subcategory")) == "reliability", "高级页默认二级分类错误")
	_expect(advanced.get("_diagnostic_bundle_button") is Button, "高级可靠性页缺少脱敏诊断包入口")
	_expect(advanced.get("_diagnostic_open_button") is Button, "高级可靠性页缺少诊断目录入口")
	_expect(advanced.get("_diagnostic_bundle_status") is Label, "高级可靠性页缺少诊断包状态指示")
	...
	var key_input := provider.get("_provider_api_key_input") as LineEdit
	var base_input := provider.get("_provider_base_url_input") as LineEdit
	var model_input := provider.get("_provider_model_input") as LineEdit
	var save_button := provider.get("_provider_save_button") as Button
	var clear_button := provider.get("_provider_clear_button") as Button
	...
	provider.call("_on_provider_preset_selected", 1)
	...
	provider.call("_update_provider_dirty_state")
	_expect(bool(provider.get("_provider_dirty")), "Provider 草稿没有进入未保存状态")
	var profile_controls: Dictionary = provider.get("_provider_profile_controls")
	...
	var fallback_controls: Dictionary = provider.get("_provider_fallback_controls")
	_expect(fallback_controls.size() == 6, "聊天、视觉、嵌入、重排序、语音识别或语音合成候选链编辑器不完整")
	var fallback_drafts: Dictionary = provider.get("_provider_fallback_drafts")
	var original_chat_fallbacks: Array = (fallback_drafts.get("chat", []) as Array).duplicate(true)
	provider.call("_add_provider_fallback_candidate", "chat")
	fallback_drafts = provider.get("_provider_fallback_drafts")
	_expect((fallback_drafts.get("chat", []) as Array).size() == original_chat_fallbacks.size() + 1, "聊天备用候选没有加入草稿")
	_expect(bool((provider.get("_provider_fallback_dirty") as Dictionary).get("chat", false)), "聊天候选链没有进入未保存状态")
	...
	_expect(
		bool(provider.call("_is_provider_insecure_http_error", "remote HTTP provider requires allow_insecure_http")),
		"公网 HTTP 错误没有触发安全确认"
	)
	_expect(
		str(provider.call("_localized_provider_error", "provider base URL must use http or https")).contains("http://"),
		"Provider 地址错误没有转换为中文提示"
	)
	provider.call("_request_provider_insecure_http_confirmation", "vision")
	var http_dialog := provider.get("_provider_http_confirmation_dialog") as ConfirmationDialog
	...
	http_dialog.hide()
	provider.call("_cancel_provider_insecure_http")
	...
	provider.call("_update_provider_profile_dirty", "vision")
	var profile_dirty: Dictionary = provider.get("_provider_profile_dirty")
	_expect(bool(profile_dirty.get("vision", false)), "视觉 Provider 草稿没有进入未保存状态")
	var proxy_controls: Dictionary = provider.get("_provider_proxy_controls")
	...
	provider.call("_update_provider_proxy_dirty")
	provider.call("_sync_provider_proxy_url_state")
	_expect(bool(provider.get("_provider_proxy_dirty")), "代理草稿没有进入未保存状态")
	...
	key_input = provider.get("_provider_api_key_input") as LineEdit
	base_input = provider.get("_provider_base_url_input") as LineEdit
	...
	profile_controls = provider.get("_provider_profile_controls")
	...
	_expect(knowledge.get("_rag_document_title") == null, "AI 页面仍堆叠显示知识库编辑器")

	panel.call("_switch_settings_category", "knowledge")
	var rag_controls: Dictionary = knowledge.get("_provider_rag_controls")
	_expect(not rag_controls.is_empty(), "RAG 配置控件不存在")
	(rag_controls.get("enabled") as CheckBox).button_pressed = true
	knowledge.call("_update_provider_rag_dirty")
	_expect(bool(knowledge.get("_provider_rag_dirty")), "RAG 草稿没有进入未保存状态")
	(knowledge.get("_rag_document_title") as LineEdit).text = "诊断知识"
	(knowledge.get("_rag_document_text") as TextEdit).text = "这是一段不会提交到模型的诊断知识。"
	_expect(knowledge.get("_rag_document_source") is LineEdit, "知识文档来源编辑器不存在")
	_expect(knowledge.get("_rag_document_filter") is LineEdit, "知识文档筛选框不存在")
	var scope_filter := knowledge.get("_rag_scope_filter") as OptionButton
	_expect(scope_filter != null, "知识文档作用域分类不存在")
	knowledge.set("_rag_documents", [
	...
	knowledge.call("_select_rag_scope_filter", "ling")
	var document_list := knowledge.get("_rag_document_list") as VBoxContainer
	...
	var batch_controls: Dictionary = knowledge.get("_rag_batch_controls")
	_expect(not batch_controls.is_empty(), "知识库批量操作控件不存在")
	(ling_row.get_child(0) as CheckBox).button_pressed = true
	_expect((knowledge.get("_rag_selected_document_ids") as Dictionary).has("ling-doc"), "批量选择没有记录文档 ID")
	_expect(knowledge.get("_rag_url_input") is LineEdit, "网页导入输入框不存在")
	var import_scope := knowledge.get("_rag_import_scope") as OptionButton
	_expect(import_scope != null, "文件与网页共用的导入作用域不存在")
	import_scope.select(1)
	_expect(str(knowledge.call("_selected_rag_import_scope")) == "ling", "批量文件导入没有读取仅小玲作用域")
	_expect(knowledge.get("_rag_search_query") is LineEdit, "检索测试输入框不存在")
	_expect(knowledge.get("_rag_file_dialog") is FileDialog, "批量文件导入对话框不存在")

	panel.call("_switch_settings_category", "general")
	_expect(knowledge.get("_rag_document_title") == null, "基础设置页仍显示知识库编辑器")
	...
	proxy_controls = provider.get("_provider_proxy_controls")
	...
	panel.call("_switch_settings_category", "knowledge")
	_expect((knowledge.get("_rag_document_title") as LineEdit).text == "诊断知识", "分类切换丢失了知识文档草稿")
	import_scope = knowledge.get("_rag_import_scope") as OptionButton
	_expect(str(import_scope.get_item_metadata(import_scope.selected)) == "ling", "分类切换丢失了导入作用域")
	scope_filter = knowledge.get("_rag_scope_filter") as OptionButton
	...
	_expect((knowledge.get("_rag_document_title") as LineEdit).text == "诊断知识", "主题重建丢失了 RAG 文档草稿")
	panel.call("_switch_settings_category", "ai")
	key_input = provider.get("_provider_api_key_input") as LineEdit
	profile_controls = provider.get("_provider_profile_controls")
	vision_controls = profile_controls.get("vision", {})
	...
	key_input.text = ""
	vision_key.text = ""
	(provider.get("_provider_fallback_drafts") as Dictionary)["chat"] = original_chat_fallbacks
	(provider.get("_provider_fallback_dirty") as Dictionary)["chat"] = false
	panel.call("_switch_settings_category", "knowledge")
	(knowledge.get("_rag_document_text") as TextEdit).text = ""
	(knowledge.get("_rag_document_title") as LineEdit).text = ""
